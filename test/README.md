...
- 沉浸模式的图片外区域固定为毛玻璃，用户背景选择只作用于图片区域；右上角关闭按钮必须常驻，普通窗口最大化请求必须重新进入沉浸。
- 设置窗口在沉浸图片上方移动后，主窗口必须保持无边框、响应和稳定合成；退回普通窗口后不保留沉浸 Alpha 状态。
...
- 自动化方式：`YeImageViewerTests` 校验 Picasa 示例尺寸、横竖图策略、退回客户区、沉浸背景分层、关闭按钮、设置控件无重叠、滚轮组合键方向和优先级、图标渲染、旋转状态重载和 Esc 决策优先级；`runTests.ps1` 验证 EXE 版本为 `1.36.9.0` 与嵌入 Commit ID，并在真实窗口中回归 Ctrl 滚轮切图、普通滚轮缩放、无边框启动、图片区不退回、背景退回、交互恢复、标题栏关闭、最大化进入沉浸、常驻关闭按钮、设置窗口跨图片移动、固定窗口切图、全屏、普通窗口 Esc 以及工具条悬停。
