...
- 单击图片或按 `Esc` 退回首次打开图片尺寸对应的带边框窗口，之后切换图片不改变外框；每张图片继续采用自己的沉浸预览百分比，不随大窗口拉伸。
- “记住最后使用的显示器”默认开启；设备名匹配忽略大小写，目标显示器断开或选项关闭时回退主显示器，负坐标窗口位置可往返恢复并被限制在工作区内。
- 自动化方式：`YeImageViewerTests` 校验 Picasa 示例尺寸、横竖图策略、退回客户区、图标渲染、旋转状态重载和 Esc 决策优先级；`runTests.ps1` 验证 EXE 版本为 `1.36.6.0`，并在真实窗口中回归无边框启动、点击退回、固定窗口切图、全屏、最大化、普通窗口 Esc 以及工具条悬停。
