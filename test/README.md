...
- 自动化方式：`YeImageViewerTests` 校验 Picasa 示例尺寸、横竖图策略、退回客户区、设置控件无重叠、图标渲染、旋转状态重载和 Esc 决策优先级；`runTests.ps1` 验证 EXE 版本为 `1.36.7.0` 与嵌入 Commit ID，并在真实窗口中回归无边框启动、图片区不退回、背景退回、交互恢复、标题栏关闭、固定窗口切图、全屏、最大化、普通窗口 Esc、设置窗口以及工具条悬停。
