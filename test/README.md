...
- 初始窗口固定为 4:3，高度取目标显示器工作区的 90%，宽度不足时按工作区宽度的 90% 反算高度；小图保持 100%，大图等比缩小，切换图片不改变窗口尺寸。
- “记住最后使用的显示器”默认开启；设备名匹配忽略大小写，目标显示器断开或选项关闭时回退主显示器，负坐标窗口位置可往返恢复并被限制在工作区内。
- 自动化方式：`YeImageViewerTests` 校验精确布局、图标渲染、旋转状态重载、初始窗口比例和 Esc 决策优先级；`runTests.ps1` 验证 EXE 版本为 `1.36.5.0`，并在真实窗口中回归初始尺寸、全屏、最大化、普通窗口 Esc 以及工具条悬停。
