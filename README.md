...
当前版本：**v1.36.10** · [修改记录](CHANGELOG.md)
...
打开图片时默认进入覆盖当前显示器工作区的无边框沉浸预览。逐像素 Alpha 黑色遮罩约为 60% 不透明度，桌面仍可透出，图片本身保持不透明，不再使用 Windows 毛玻璃；退回普通窗口并拉大窗口后，非图片区域继续使用相同效果。小图按当前 DPI 下的 100% 逻辑尺寸显示；横图最多占工作区宽度的 90% 和高度的 82.5%，竖图只在宽度超过 90% 时缩小，超出高度的部分可上下拖动查看。
单击图片或按 `Esc` 会退回带边框窗口，客户区按照首次打开图片的显示尺寸确定；之后切换上一张、下一张不会改变窗口外框，各图片仍沿用自己的沉浸预览百分比，不会因为外框较大而被强行拉伸。旋转方向会按图片路径自动保存。
